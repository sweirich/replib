-- | Permutations 
module Data.RepLib.Bind.Perm (Perm, empty, single, 
                         join, apply, support, isid, joins) where
...
data Perm a = Empty
            | Single a a
            | Join (Perm a) (Perm a) deriving (Show)

instance Ord a => Eq (Perm a) where
   p1 == p2 = all (\x -> apply p1 x == apply p2 x) (support p1)
              && support p1 `seteq` support p2

-- | Create an identity permutation which leaves its argument unchanged
empty :: Eq a => Perm a
empty  = Empty   

-- | Create a singleton permutation
-- "single x y" exchanges x and y but leaves all other values alone
...
-- | The list of objects that are changed by this permutation
...
-- | Join a list of permutations together
